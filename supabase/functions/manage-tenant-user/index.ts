...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Verificar que es platform_admin usando service_role (evita RLS)
    const supabaseAdmin = createClient(
      Deno.env.get("SUPABASE_URL")!,
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
      { auth: { autoRefreshToken: false, persistSession: false } }
    ...
    if (action === "list_users") {
      ...
      // Obtener tenant_users del tenant
      const { data: tenantUsers, error: tuError } = await supabaseAdmin
        .from("tenant_users")
        .select("user_id, rol, created_at")
        .eq("tenant_id", tenant_id)
      ...
      // Obtener datos de auth.users para cada usuario
      const userIds = tenantUsers.map(tu => tu.user_id);
      const usersWithAuth = await Promise.all(
        userIds.map(async (uid) => {
    ...
    if (action === "reset_password") {
      ...
      return new Response(JSON.stringify({
        ...
        new_password: tempPassword,
        user_email: updatedUser.user?.email,
        nota: "⚠️ Muestra esta contraseña al administrador de la empresa. No se volverá a mostrar.",
      }), {
        status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "update_email") {
      ...
      const { data: updatedUser, error: updateError } = await supabaseAdmin.auth.admin.updateUserById(
        ...
        {
          email: new_email,
          email_confirm: true, // Confirmar automáticamente sin requerir verificación
        }
      );
    ...
    if (action === "create_admin") {
      ...
      if (createError) {
        if (createError.message?.includes("already")) {
          // El usuario ya existe — solo vincularlo al tenant
          ...
          return new Response(JSON.stringify({
            error: `El correo ${new_email} ya está registrado. Use "Actualizar correo" o proporcione uno diferente.`
          }), {
            status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      return new Response(JSON.stringify({
        ...
        nota: "⚠️ Entrega estas credenciales al administrador de la empresa. No se volverán a mostrar.",
      }), {
        status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
